...
// sent after native-ready only when something differs from the engine's
// own defaults (mask 0, NPU allowed).
...
test('defaults: no CONFIG is sent after ready, and the ready message\'s backends land in the snapshot', async () => {
  ...
  port.emit(READY);
  await client.ready;
  assert.equal(port.sent.length, 0, 'nothing differs from the engine defaults');
...
test('NATIVE_NPU 0 sends a CONFIG with the NPU flag cleared even at mask 0', async () => {
  nc.setNativeCpuMask(0);
  nc.setNativeNpu(0);
  const port = makeFakePort();
  const client = nc.createNativeClient(port);
