...
| Identity memory | BLUR DIRECTION ONLY: cos ≥ 0.85 (MEM_SIM_FLAG) re-covers a remembered certain-flagged face; clears are never memorized | memory may only ADD coverage — a false match costs over-blur, never exposure | MEASURED 2026-08-25 (128 same-person + 65 same-frame different-person pairs): same-person median 0.90, but DIFFERENT people scored ≥0.6 in 32% of pairs and ≥0.9 in 17% — faceres does not separate identity at our crop quality at ANY threshold, so the clear-inherit path was deleted |
| Identity memory update | cos ≥ 0.85 (MEM_SIM_UPDATE), exemplars not centroids (2-3 raw descriptors, match = max) | a blended centroid could be walked toward the wrong face by repeated near-threshold updates | review A2/C6 |
