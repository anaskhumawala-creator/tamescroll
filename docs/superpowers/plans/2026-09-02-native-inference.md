...
- [x] Agent reports. Read `BRIDGE-REPORT.md`. **Decision:** transport = ArrayBuffer port if supported and its transfer p50 < 10ms at 256x256 RGBA; else base64 JSI. Record the choice and the number in Loop state.
- [x] Review the Kotlin diff. Keep the port plumbing (Task 2 builds on it); delete the base64 bench path unless it won. Commit.
...
- [x] **Step 1: Project.** `spikes/native/bench-android/` with `settings.gradle.kts`, `app/build.gradle.kts` (compileSdk 36, minSdk 24, deps `org.tensorflow:tensorflow-lite:2.16.1`, `org.tensorflow:tensorflow-lite-gpu:2.16.1`, `org.tensorflow:tensorflow-lite-gpu-api:2.16.1`; `aaptOptions { noCompress += "tflite" }`), `BenchActivity.kt`:
...
- [x] **Step 2: Build + run.** `JAVA_HOME` as in the recipe; `./gradlew :app:assembleDebug -q`; `adb -s 1ec2c48e0621 install -r`; `adb shell am start -n app.tamescroll.bench/.BenchActivity`; `adb logcat -d | grep TSBENCH` (also grep `tflite` for delegate fallback lines). Repeat the whole run TWICE (the device throttles; report both).
- [x] **Step 3: GPU-REPORT.md** with the table, delegate-coverage lines, and the sum for ONE verdict pass in the app's shape: MoveNet once + BlazeFace once + faceres × 2 crops. **Gate for Task 2:** that sum on the GPU delegate ≤ 300ms steady-state on the Redmi (today's WebGL equivalent is ~700-900ms). If it is not, stop, write why in Loop state, and try f16 + `setPrecisionLossAllowed(true)` / NNAPI delegate (`NnApiDelegate()`) as the second attempt before declaring the approach dead.
- [x] Commit `spikes/native/bench-android/` (source only; add `build/`, `.gradle/` to `.gitignore`).
...
- [x] On the Redmi, with a debug hook `window.__TS_NATIVE_PARITY` (flag-gated, nothing in the app sets it, a test pins that), feed 20 banked thumbnails (the ids in `spikes/gauntlet/probe_faceres_parity.py` — reuse its serving trick over `adb reverse tcp:8899`) through BOTH paths: Worker (WebGL) and native. Compare: BlazeFace boxes after NMS (IoU ≥ 0.9 per matched box, same count), MoveNet persons admitted (same count, keypoint max within 0.02), faceres gender raw within 0.03, age within 1.0 year, descriptor cosine ≥ 0.98. **Gate (RESTATED 2026-09-02 after ledger K11 — the original wording named only the two FLAG bars, and a player patch is not decided at either):** 0 decision flips across all faces at EVERY bar the player path reads — the flag bars `GENDER_MIN_SCORE` 0.25 / `GENDER_IMAGE_MIN_SCORE` 0.4, AND the bars that decide whether a covered person is REVEALED: `clearBarFor` (`GENDER_CLEAR_SCORE` 0.45 / `GENDER_CLEAR_SCORE_FEMALE` 0.35), the child gate `GENDER_CHILD_MASS` 0.25, and `NULL_MINT_NM_FLOOR` 5. Plus `faceCountMismatchFrames` 0 (K1: a lost DETECTION is invisible to every score comparison). The probe reads those bars off the RUNNING bundle (`__TS_GAZE_IDS.cfg`) and REFUSES to score them if the hook is absent — it may never re-type them (G1/G5/G9). Report the split, never a single number: e.g. "0 flag-bar flips, 2 of 24 clear-bar/child-gate flips, all in the covering direction". A flip in the EXPOSING direction at any bar fails the gate outright; covering-direction flips are recorded and judged. (The 2026-08-31 uint8 requant was refused at 8/100 flips; note K12 — at n=24 the 95% upper bound is 11.7%, which CONTAINS 8%, so this corpus cannot yet distinguish this build from that one. ~37 clean reads pushes the bound under 8%.) If f16 flips any, use f32 for that model and re-measure Task 1's sum.
- [x] Commit the probe + report.
...
- [x] Build, install on the Redmi, `python spikes/gauntlet/probe_latency_ab.py 9227 native 150 --delay`, TWICE. Bank as `latency-ab-native.json` / `-native2.json`. Compare with `latency-ab-stageB5.json` (1092). **Gate:** verdict p50 ≤ 350ms and gap p50 ≤ 800ms with `worker.backend` reading `native-gpu`, coverage within 0.03 of 1092, rAF ≥ 1092's 34. Also read `nativeFallback`/`nativeFailed` (must be 0/0) and the delay arm (entry lag should FALL — the verdict now lands well inside the 1s delay, so `delayVerdictLate` should approach 0).
- [x] Kill-switch check: push `NATIVE_INFER: 0` to a scratch tuning (or set it via `__TS_TUNING` in page), confirm `worker.backend` returns to `webgl` and the run is 1092's numbers. Restore.
- [x] Record both rows in Loop state.

### Task 6: Fullscreen + miniplayer + seek with native on (Sonnet, timebox 45m)

- [x] Real input events over CDP (`Input.dispatchMouseEvent` for the fullscreen button — see loop 18's note about asserting the button is hittable), drag-to-mini via `probe_mini_yt.py`'s gesture: the native path keeps answering across both (`nativeUsed` rising, patches present, 0 outside the player). Seek: refill + first native verdict within 3.5s.

### Task 7: Adversarial critic (Opus)

- [x] Launch the critic against the diff since `f422be2` and the raw JSON in `spikes/native/` + `spikes/gauntlet/latency-ab-native*.json`. Deliverable `docs/critic/phase-j.md`; rows into `ledger.md`; fix at source; `critic-gate` must exit 0. An open EXPOSURE row blocks Task 8.

### Task 8: Release 1093

- [x] Bump `appupdate.rs` CURRENT_VERSION_CODE 1093, `tauri.conf.json` 0.1.93, `tauri.properties` (gitignored) 1093/0.1.93. Build, install on the Redmi, one last `probe_latency_ab` row, `gh release create app-v0.1.93`, `node scripts/gen-app-manifest.mjs`, commit+push manifest, re-download the served APK and compare its sha to the manifest, `isDraft` false. Update CLAUDE.md session state with the table. `NOTICE` carries TFLite.
...
- **Current task:** PLAN COMPLETE (2026-09-02 19:40). Every step of Tasks 0-8 is done and ticked; 1094 is published (app-v0.1.94, sha 1d98f270), the Redmi runs it, critic-gate 0 blocking. A loop run that reads this replies "plan complete" and does nothing else (its brief, last line). What is left is a NEW plan, not this one: `VERDICT_DUTY` 1.5 measured on the hoisted clock (expect gap ~600, +25% GPU) before an OTA push; K10 (`delayVerdictLate` 214 vs 0); K12 corpus growth (~37 clean reads) if the fp16 faceres parity claim needs power; the fullscreen delay canvas with the presenter attached; iOS (CoreML) and desktop (ORT/DirectML) on the same port protocol. Owner ruling still open: faceres fp16 (gender 220 -> 174ms; 3 covering-direction flips over 2 of 24 reads, inert in man mode) -- ships fp16.
...
- **Blocked / needs owner:** FACERES PRECISION, and it is the sole cause of the only parity flips left. `MODEL_FP16 = setOf(2)` keeps faceres at fp16 (BlazeFace went back to fp32 for K1). Over 24 read pairs that costs 2 clear-bar flips and 1 child-gate flip on 2 of 24 reads -- all in the COVERING direction on native, and inert in his `man` mode, where a female read is opposite-gender and covered regardless. It buys gender 220 -> 182ms. Options, none needing more than a build: (1) LEAVE fp16 -- my pick, the flips only ever cover; (2) faceres to fp32 -- no flips, ~38ms back on every verdict; (3) grow the parity corpus to ~37 clean reads FIRST, because K12 shows the 95% upper bound at n=24 is 11.7%, which still contains the 8% that got the 2026-08-31 uint8 requant refused -- i.e. today "parity clean" and "as bad as the refused requant" are the same measurement.
...
- **Log:**
  - 2026-09-02 19:35 THE LETTERBOX EVIDENCE IS NOW ON THE SHIPPED APK, AND THE TWO EARLIER RUNS HAD NOT ACTUALLY ROTATED. `probe_delay_letterbox.py` on 1094 as served, phone locked PORTRAIT before the run (`delay-letterbox-1788357238.json`):

    | arm | viewport | video | canvas | fit | painted | eq per sample | worst edge |
    |---|---|---|---|---|---|---|---|
    | PORTRAIT | 392x822 | [0,48,393,221] | [0,48,393,221] | contain | [0,48,393,221] | TTTTTTT | 0px |
    | LANDSCAPE | 822x364 | [85,48,652,367] | [0,48,823,367] | contain | [85,48,652,367] | TTTTTTT | 0px |
    | PORTRAIT2 | 392x822 | [0,48,393,221] | [0,48,393,221] | contain | [0,48,393,221] | TTTTTTT | 0px |

    21 of 21 samples, `patchesOutsidePlayer` 0 throughout, and the landscape arm genuinely exercises the letterbox (player 823 wide against a 652-wide video). **A ROTATION RESTARTS THE APP AND KILLS THE CDP SOCKET** -- from a portrait-locked phone the first rotate ended the run with `ConnectionAbortedError 10053` mid-arm. The two runs banked earlier today survived only because the phone was ALREADY rotated, so the probe's `rotate()` was a no-op: **the arms that passed had not exercised a rotation at all**, which is the "a probe that measures nothing reads exactly like a clean one" failure. `reattach()` now re-forwards the devtools socket (it carries the NEW pid), reopens the tab and drives the page back to the same seek after every rotation, so the three arms are three real orientations. Same class as the earlier `last`-only defect this probe had: keeping only the final sample printed `painted == video` exactly while the verdict read FAILED, with nothing in the file saying which of 7 samples disagreed -- it banks `rows`, `eqPerSample` and `worstEdgePx` now.
  - 2026-09-02 19:15 1094 ROUND CLOSED (phase-k K1-K13 landed, two device defects found by the round's own probes, all on the Redmi). Chain on the shipping build, `probe_latency_ab.py --delay` 150s same video/seek: **verdict p50 355 / p95 ~1200, gap 805 / 2353, verdicts 135 (15 dropped), positions 90, rAF 36-40, coverage 0.577-0.602, entry lag p50 33ms, exit hang p50 0 frames, nativeErrors 0, nativeDead 0** (`latency-ab-native1094.json`; `-prefit.json` is the same config one build earlier: gap 798, coverage 0.602). K1: BlazeFace back to fp32 (`MODEL_FP16 = setOf(2)`), parity rerun `native-parity-1788354123.json`: **faceCountMismatchFrames 0**, faces 24/24 IoU p50 0.998, gender raw diff p50 0.0025, cosine min 0.995, native frame 342 / gender 174ms vs worker 701 / 473. K3/K8: the yield gate is gone (24ms for a third of the position passes); the hoist alone carries 1213 -> 805. K4 RE-PRICED on the hoisted clock: `CUT_PERSON_LOOK` 1 vs planted 0 = gap 805 vs 803, cost 355 vs 384, personPassSkipped 53 vs 63, coverage 0.577 vs 0.575 -- the +15% was the old clock's; stays 1. K5: fail-safe probe reads at the kill; two clean runs HOLD (`native-failsafe-1788355304.json` 87 -> at-kill 91 -> after 91 flat, 14/20 -> 9/20; `-1788355994.json` 93 flat, 14/20 -> 7/19). K2: findings 26 corrected (time-weighted native 0.643 vs worker 0.673; the attribution rests on the geometry bench + exit hang alone). K6/K7/K10 recorded. **DEFECT 1, EXPOSURE IN LANDSCAPE/FULLSCREEN, found by the A/B running with the phone lying on its side:** the delay canvas was `width/height:100%` of `#movie_player` and the video letterboxes inside it when the player is wider (landscape: video [85,48,652,367] in a canvas [0,48,823,367]) -- a 16:9 frame drawn at 2.24:1 with every patch positioned off the VIDEO rect, i.e. beside the face. Fixed with `object-fit: contain` on the canvas (a replaced element); `probe_delay_letterbox.py` rotates the device and compares the PAINTED rect with the video rect: `delay-letterbox-1788355188.json` and `-1788355596.json` read painted == video in portrait, landscape and back, worst edge 0px. `probe_latency_ab.py` geometry now reads the painted rect too. **DEFECT 2, tap-to-restore on the mini player was dead in portrait** (`native-task6-1788355877.json`: tap on the cover, mini stays): an UNCLAIMED touch still ran `endDrag -> place -> parked`, which cleared and rewrote the transform with the .22s transition live, so the click the browser synthesizes 35ms after touchend hit-tested `<html>` while the container animated in from the full position; it only ever worked in landscape because the mini's centre sits inside the full player's rect there. Traced event by event (touchstart/touchend on ts-mini-cover, click on HTML). `onUp` now returns on an unclaimed touch; structural test red against HEAD. Task 6 rerun on that build below. A second session (disconnect-21) shared the checkout and the Redmi for ~20 minutes: it landed K11-K13 (parity gate re-quoted at `clearBarFor` / child gate via a diagnostic `cfg` block, coverage bench fixed), and its overlapping probe contaminated one Task 6 run (`native-task6-1788355470.json`, windowed arm at t=17, superseded). gaze 698/698, cargo 61/61, critic-gate 0 blocking (K10, K12 open NITs).
  - 2026-09-02 19:20 POST-1093 ROUND, HIS CALL DELEGATED ("its all technical man, you take the call"). Redmi, `probe_latency_ab.py --delay`, 150s, same video/seek, every row on the 1093 code unless named:
    ...
    | fp16 BlazeFace/faceres, fp32 MoveNet | 419 / 1131 | 1200 / 2394 | 112 | 75 | 44.1 | 0.583 | parity vs worker: face IoU p50 0.997 min 0.984, gender raw diff p50 0.0025 max 0.018, labels 22/24 (both misses raw 0.50 coin flips), cosine p50 0.9990 min 0.995; gender 220 -> 182ms. **THIS ROW'S PARITY CLAIM IS WITHDRAWN AS STATED (K1, K11, K13):** fp16 BlazeFace LOSES the t=90 face outright (`faceCountMismatchFrames` 1, and that face is the frame's whole cover), and at the bars the player decides at there are 2 clear-bar + 1 child-gate flips over 2 of 24 reads, all covering. BlazeFace is fp32 at HEAD; faceres stays fp16 and still produces all three bar flips. |
