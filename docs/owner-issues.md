...
|---|----------|-----------------------------------|--------|----------------------|
| 12 | 08-23 | "why did ad come up" (desktop watch) | IN-PROGRESS | Root-caused: hard-nav path serves pre-roll because only 5 of ~12 needed scriptlets ship; SPA path strips ad but stalls 4.4s (profiling agent, 6/6 runs, transplant-proven). Fix = clean-room implement missing scriptlets. |
...
| 9 | 08-22/23 | App "loads a lot" after clicking a video | IN-PROGRESS | Same root cause as #12 (4.4s scriptlet stall on every SPA watch click — the only click path). Secondary: tfjs webgl init blocks main thread 1-2.2s on hard loads (worst in off mode: NSFW model loads immediately); bundle parsed 3x per hard nav (~200ms each). |
