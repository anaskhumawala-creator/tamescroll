...
# Several queries, because one search is one population. The reference
# gender comes from the model's own full-resolution read, so a mix
# matters only for covering both directions of the decision.
QUERIES = (sys.argv[2] if len(sys.argv) > 2 else
           'podcast interview,news anchor,makeup tutorial,ted talk').split(',')
...
ids = []
for q in QUERIES:
    t.cmd("Page.navigate", url="https://m.youtube.com/results?search_query=" + q.strip().replace(' ', '+'))
    time.sleep(20)
    got = t.eval("""(function(){
      var out=[], seen={};
      ...
      return JSON.stringify(out.slice(0,%d));})()""" % PER)
    got = json.loads(got) if isinstance(got, str) else []
    print(q.strip(), len(got))
    for g in got:
        if g not in ids:
            ids.append(g)
